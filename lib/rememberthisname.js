...
// WARNING: to keep this demo simple, collections are just cached in memory and ids are generated
// sequentially. In real usage, you'd want to store this data externally (e.g. redis), use proper
// id generation, ensure users can only access their own collections, etc.
...
function getModelsToRender(collectionId, callback) {
    ...
    if (collection) {
        console.log("getModelsToRender("+collection.length+")")
        collection.each(function(model) { sendModelToClient(model, callback); });
    } else {
        console.log("getModelsToRender(null)")
        callback(null);
    }
}
